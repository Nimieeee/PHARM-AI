...
async def check_structure():
    ...
    try:
        ...
        result = await supabase.table('conversations').select('*').limit(5).execute()
        if result.data:
        ...
        try:
            ...
            # Clean up
            await supabase.table('documents').delete().eq('document_id', 'test_structure_check').execute()
        ...
        result = await supabase.table('documents').select('*').limit(1).execute()
        if result.data:
